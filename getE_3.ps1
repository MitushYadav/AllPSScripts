...
$reqDep = get-cmdeployment | where-object { $_.FeatureType -in 1,2 -and $_.NumberTargeted -gt 0 -and $_.CollectionName -cnotlike "*USR*" -and $_.SoftwareName -cnotlike "*NOSHORTCUT" }
...
foreach($dep in $reqDep) {
    if ($dep.FeatureType -eq 1) {
        #Application
        $app = Get-CMApplication -ID $dep.CI_ID
        $details = @{}
        ...
        #Since there can be multiple deployments for the same software name
            ...
            If($app.LocalizedDisplayName -like "ALO*") { 
              $details.Type = 'Scripted'
              $details.SCCMName = $app.LocalizedDisplayName.Substring( }
        ...
        #Package
        $pack = Get-CMPackage -ID $dep.PackageID
